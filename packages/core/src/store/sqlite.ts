...
export class SqliteStore implements DBStore {
  ...
  async getNodes(): Promise<FerretNode[]> {
    return this.db!.prepare('SELECT * FROM ferret_nodes').all() as FerretNode[];
  }

  async getNodesByStatus(status: NodeStatus): Promise<FerretNode[]> {
    return this.db!.prepare('SELECT * FROM ferret_nodes WHERE status = ?').all(status) as FerretNode[];
  }

  async getContracts(): Promise<FerretContract[]> {
    return this.db!.prepare('SELECT * FROM ferret_contracts').all() as FerretContract[];
  }
  ...
  async getDependencies(): Promise<FerretDependency[]> {
    return this.db!.prepare('SELECT * FROM ferret_dependencies').all() as FerretDependency[];
  }
  ...
  async getReconciliationLogs(): Promise<FerretReconciliationLog[]> {
    return this.db!.prepare('SELECT id, node_id, triggered_by, resolved_by, resolution_notes FROM ferret_reconciliation_log').all() as FerretReconciliationLog[];
  }
  ...
  async getPlacementDecisions(): Promise<FerretPlacementDecision[]> {
    return this.db!.prepare('SELECT id, node_id, placed_by, reasoning FROM ferret_placement_decisions').all() as FerretPlacementDecision[];
  }
}